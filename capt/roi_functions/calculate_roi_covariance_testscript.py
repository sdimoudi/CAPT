	...
	for i in range(mapPos0):
		...
		if roi_axis!='x':
			roi_cov_yy = numpy.zeros(roi_ones.shape)
                ...
                t1 = time.time()
                roi_cov_nores = Parallel(prefer='threads', n_jobs=31)(delayed(inner_loop)(j, i, roi_ones_arange, mm_subapPos,sa_mm,sb_mm, mm, subap1_comb_shift, subap2_comb_shift, roi_axis, mapping_type, shwfs_centroids, wfs1_n_subap, wfs2_n_subap, roi_cov_xx, roi_cov_yy) for j in range(1, 32))
                t2 = time.time()
                ...
                t1 = time.time()
                roi_cov_nores = Parallel(prefer='threads', n_jobs=16)(delayed(inner_loop)(j, i, roi_ones_arange, mm_subapPos,sa_mm,sb_mm, mm, subap1_comb_shift, subap2_comb_shift, roi_axis, mapping_type, shwfs_centroids, wfs1_n_subap, wfs2_n_subap, roi_cov_xx, roi_cov_yy) for j in range(32,48))
                t2 = time.time()
